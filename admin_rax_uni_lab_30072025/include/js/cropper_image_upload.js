...
function multiple_pdf_upload(obj, field, cropper) {
	...
	const file = jQuery(obj).get(0).files[0];
	...
	const image_count = jQuery('#' + field + '_view').find('.' + field + '_div').length;
	if (image_count >= 5) {
		jQuery('#' + field + '_view').before('<div class="alert alert-danger w-100 text-center">Maximum PDF upload count is 5</div>');
		return;
	}
	...
	jQuery.ajax({
		...
		success: function (result) {
			const banner_div_id = `<div class="${field}_div col-lg-2 col-md-4 col-5"><div class="form-group w-100 px-3 py-3 cover">${result}</div></div>`;
			jQuery('#' + field + '_view').append(banner_div_id);
		},
		error: function (xhr) {
			jQuery('#' + field + '_view').before('<div class="alert alert-danger w-100 text-center">Upload failed</div>');
		}
	});
}
