...
(function( $ ){
	...
	var methods = {
		...
		imageWidget: function(){
			...
			var container = $('<div/>',{'class':"tabbable tabs-left mx-3 w-100"});
			var navTabs = $('<ul/>',
									{ class: "nav nav-tabs image_get_link mb-2 mr-0 w-100"
							}).append($('<li/>',
										{ class:"col-sm-6 active"
									}).append($('<a/>',{
											"href":"#uploadImageBar_" + _idSuffix,
											"data-toggle":"tab"
										}).html("From Computer")
							)).append($('<li/>', {class:"col-sm-6"}).append($('<a/>',{
											"href":"#imageFromLinkBar_" + _idSuffix,
											"data-toggle":"tab"
										}).html("From URL")));

			var tabContent 		= $("<div/>", {class:"tab-content image_content mx-3"});
			...
			var statusBar = $("<div/>", {	id : "statusbar_" + $(this).attr("id"),
	       	...
	       	if(settings['status_bar']){
				editor.keyup(function(event){
					...
					$(editor_Content).data("statusBar").html('<div class="badge bg-secondary">'+'Words : '+wordCount+'</div>');
					$(editor_Content).data("statusBar").append('<div class="badge bg-secondary">'+'Characters : '+charCount+'</div>');
            	});
	        }	        
		...
		createTableContext: function(event,cMenuUl){
			...
			cMenuUl.append($('<li/>',{class:"dropdown-submenu bs5-rewrite-needed",css:{display:"block"}})
       			...
       			)).append($('<li/>',{class:"dropdown-submenu bs5-rewrite-needed",css:{display:"block"}})
		...
		createMenuItem: function(itemSettings, options, returnElement){
			...
			if(itemSettings["select"]){
				...
				var menuWrapElement = $("<a/>",{href:'javascript:void(0)', class:'btn btn-secondary'});
				var menuElement = $("<i/>");
				if(itemSettings["icon"])
					menuElement.addClass(itemSettings["icon"]);
				else
		...
		showMessage: function(target,message){
			...
			setTimeout(function() { $('.alert').html('close'); }, 3000);								
		},
