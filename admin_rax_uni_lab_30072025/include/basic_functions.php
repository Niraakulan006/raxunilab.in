<?php
	include("rax_uni_lab_30072025.php");
	class Basic_Functions extends db {
		...
		public function getProjectTitle() {
			...
			// }
			if(empty($project_title)) { $project_title = "Rax Uni Lab"; }
		...
		public function automate_number($table, $column) {
			...
			if(!empty($table) && $table == $GLOBALS['vegetable_preparationsheet_table']) {
				$prefix = 'VPS';
			}
			elseif(!empty($table) && $table == $GLOBALS['grocery_preparationsheet_table']) {
				$prefix = 'GPS';
			}
			elseif(!empty($table) && $table == $GLOBALS['order_form_table']) {
				$prefix = 'ORD';
			}
			...
			$select_query1 = "SELECT ".$column." as last_number FROM ".$table." where (".$column."!='".$GLOBALS['null_value']."' && ".$column."!='') ORDER BY id DESC LIMIT 1";
			...
			if(strpos($last_number, '/') !== false){
				...
				if(!empty($prefix)){
					if(!empty($table) && $table == $GLOBALS['vegetable_preparationsheet_table']) {
						$last_number = str_replace("VPS","",$last_number);
						$last_number = trim($last_number);
					}
					elseif(!empty($table) && $table == $GLOBALS['grocery_preparationsheet_table']) {
						$last_number = str_replace("GPS","",$last_number);
						$last_number = trim($last_number);
					}
					elseif(!empty($table) && $table == $GLOBALS['order_form_table']) {
						$last_number = str_replace("ORD","",$last_number);
						$last_number = trim($last_number);
					}
				}
						...
						if(preg_match("/^\d+$/", $last_insert_id)) {
							if(!empty($custom_id)) {
								...
								if($last_insert_id < 10) {
									$custom_id_value = date("dmYhis")."_0".$table."_".$last_insert_id;
								}
								else {
									$custom_id_value = date("dmYhis")."_".$table."_".$last_insert_id;
								}
		...
		$result = $this->getQueryRecords($GLOBALS['role_permission_table'], $select_query);
		...
		if(!empty($company_id)){
			$where = "bill_company_id = '".$company_id."'";
		}
		if(!empty($role_id)){
			if(!empty($where)) {
		...
		if(!empty($permission_module)){
			if(!empty($where)) {
				$where = $where." AND module = '".$permission_module."'";
			}
			else {
		...
		if(!empty($where)) {
			$select_query = "SELECT id FROM ".$GLOBALS['role_permission_table']." WHERE ".$where." AND deleted='0'"; 
		}
		if(!empty($select_query)) {
		...
		public function getOtherCityList($district) {
			...
			$select_query = "SELECT DISTINCT(others_city) as city FROM ((".$company_query.")) as g";
		...
		public function accessPageAction($bill_company_id,$role_id,$permission_module){
			...
			if(!empty($where)) {
				$select_query = "SELECT module_actions FROM ".$GLOBALS['role_permission_table']." WHERE ".$where." AND deleted='0'"; 
			}
			if(!empty($select_query)) {
				$list = $this->getQueryRecords($GLOBALS['role_permission_table'], $select_query);
				if(!empty($list)) {
