<?php 
    ...
    $home_banner_record = array(); $home_banner_record_count = 0; $home_banner_id = "";
    $home_banner_record = $obj->getTableRecords($GLOBALS['home_banner_table'],'','','');
    if(!empty($home_banner_record)){
        $home_banner_record_count = count($home_banner_record);
        foreach($home_banner_record as $data){
            if(!empty($data['home_banner_id'])){
                $home_banner_id = $data['home_banner_id'];
            }
        }
    }
?>
...
<script>
    $(document).ready(function(){
        ...
        <?php 
        if(empty($home_banner_record_count)){ ?>
            ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '');
            <?php
        }else { ?>
             ShowModalContent('<?php if(!empty($page_title)) { echo $page_title; } ?>', '<?php if(!empty($home_banner_id)) { echo $home_banner_id; } ?>');
        <?php } ?>
